...
import { Trash2, Image as ImageIcon, Type, Square, Heading, Check, Monitor, Smartphone } from "lucide-react";
...
export default function HomepageEditorPage() {
  ...
  // Always-current refs so the single mousemove useEffect sees latest state without re-registering
  ...
  // These are re-assigned on every render — always current
    ...
    fetch("/api/page-config")
      ...
      .then((d) => {
        if (d?.desktop) {
          ...
          // Legacy single-view format — migrate to desktop
  ...
  // Single global mouse handler — uses refs so no re-registration needed
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (heightDragRef.current) {
        ...
        setHeightRef.current(Math.max(400, origH + delta));
      }
    };
    const onUp = () => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="hidden md:flex flex-col h-full">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center rounded-lg border overflow-hidden ml-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-sm transition-colors ${
                view === "desktop" ? "bg-black text-white" : "hover:bg-muted"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-sm transition-colors ${
                view === "mobile" ? "bg-black text-white" : "hover:bg-muted"
              }`}
            >
          ...
          <div className="ml-auto">
            <Button onClick={save} disabled={saving} className="gap-2">
              ...
              ) : saving ? "Saving…" : "Apply to Homepage"}
            </Button>
          </div>
        </div>
        ...
        <div className="flex gap-4 flex-1 min-h-0">
          ...
          <div className="w-44 shrink-0 flex flex-col gap-2 overflow-y-auto pb-4">
            ...
            {selectedComp && (
              <div className="mt-4 border-t pt-4 flex flex-col gap-3">
                ...
                {(selectedComp.type === "text" || selectedComp.type === "header") && (
                  <>
                    ...
                    <div>
                      ...
                      <input
                        type="color"
                        value={selectedComp.fontColor ?? "#000000"}
                    ...
                    <div>
                      ...
                      <div className="flex gap-1 mt-1">
                        {(["left", "center", "right"] as const).map((a) => (
                          <button
                            key={a}
                            onClick={() => updateComp(selectedComp.id, { textAlign: a })}
                ...
                {selectedComp.type === "shape" && (
                  <>
                    <div>
                      ...
                      <input
                        type="color"
                        value={selectedComp.bgColor ?? "#3b82f6"}
                ...
                {selectedComp.type === "image" && (
                  <>
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        value={selectedComp.borderRadius ?? 4}
                        onChange={(e) => updateComp(selectedComp.id, { borderRadius: +e.target.value })}
                        className="h-7 text-sm mt-1"
                      />
                    </div>
                ...
                <div>
                  ...
                  <input
                    type="color"
                    value={selectedComp.bgColor === "transparent" ? "#ffffff" : (selectedComp.bgColor ?? "#ffffff")}
                    onChange={(e) => updateComp(selectedComp.id, { bgColor: e.target.value })}
                    className="mt-1 w-full h-7 rounded cursor-pointer border"
                  />
                </div>
          ...
          <div className="flex-1 overflow-auto bg-gray-100 rounded-lg p-3">
            ...
            <p className="text-center text-xs text-muted-foreground mt-1 select-none">
              {view === "desktop" ? "Desktop" : "Mobile"} — {canvasW} × {Math.round(canvasH)} px — drag to move · corner handle to resize · pill below to adjust height
            </p>
          </div>
        </div>
      </div>
